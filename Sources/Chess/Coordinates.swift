...
extension Coordinates {
        // helper function to return an array of all cases for each rank
        // can be used in board setup

        public static func allCoordinatesFor(rank: Ranks) -> [Coordinates] {
                var result: [Coordinates] = []
                for f: Files in Files.allCases {
                        let c = Coordinates(file: f, rank: rank)
                        result.append(c)
                }
                return result
        }
